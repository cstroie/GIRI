...
### MODIFICAT — Normalizarea modalității „RM" → „IRM" (2026-07-10, la cerere)

Convenție: modalitatea se scrie **IRM** (imagistică prin rezonanță magnetică), nu „RM".
Aplicat pe tot fișierul: **14 câmpuri** din 4 capitole (Pediatrie, Aparat digestiv, Cap,
Radiologie intervențională). Validare verde.

- **Convertit:** „RM" de sine stătător (ex. „contraindicație de RM" → „…de IRM";
  enumerări „CT, RM, scintigrafie" → „CT, IRM, …") **și** compusele cu cratimă
  (`Uro-RM`→`Uro-IRM`, `colangio-RM`→`colangio-IRM`, `colangiopancreatografie-RM`→
  `…-IRM`). Regex cu delimitare pe non-literă → nu atinge „IRM" existent.
- **Neatins (acronime tehnice lipite):** `CPRM` (×10), `CRM` (×2), `spectroRM` (×3) —
  abrevieri consacrate (colangiopancreatografie-RM, colangio-RM, spectroscopie RM). Rămâne
  o mică neconcordanță formă-extinsă vs. acronim (ex. „colangio-IRM (CRM)"), acceptabilă;
  de reconfirmat dacă se dorește și `spectroRM`→`spectroIRM` (există precedentul `sialoIRM`).
...
- **DOCUMENTAT (fără modificare de date) —** analiză de acoperire a capitolului față de ACR
  Appropriateness Criteria® (Women's Imaging), RCR iRefer și CAR OB/GYN. Consemnată în
  `EDITORIAL-decisions.md §28`: ~10 goluri clare cu home în capitol (durere pelvină acută /
  torsiune anexială, sângerare uterină anormală premenopauzală, fibroame, endometrioză ca
  situație proprie, malformații mülleriene, sângerare de trimestru II/III, placenta accreta,
  hemoragie postpartum, col gravid / insuficiență cervicală, IRM fetal), goluri cu home în
  alt capitol (embolizări → RI; planșeu pelvin; cancere → Cancer) și îmbogățiri (HSG/HyCoSy
  la infertilitate, O-RADS la masă anexială, PUL). **Niciun rând adăugat** — propuneri pentru
  decizia editorilor.

## 9. Sân
_NR 903–958_

### ⛔ BREAKING — Intervenționalul mamar readus în Sân din RI (excepție de senologie) (2026-07-10, la cerere)

- Decizie editor: **senologia e specialitate proprie** → procedurile intervenționale mamare
  (Tip `I`/`A`) rămân în capitolul **Sân**, nu se mută la RI. Excepție nouă, analog „cancerul de
  sân rămâne în Sân" (vezi `CLAUDE.md` › „Decizii deja luate" și `EDITORIAL-decisions.md` §15.B).
- **Mutat** din `RI › Oncologie` în `Sân › Paciente cu simptome și semne sugestive de cancer`
  (lângă rândurile diagnostice-soră ale aceleiași situații):
  - **NR 1192** — „Diagnosticul cancerului de sân: nodul mamar…" · Biopsie percutanată (Tip `I`,
    Indicat, grad A).
  - **NR 1178** — „Adenopatie axilară suspectă" · Biopsie ganglionară (Tip `I`, Doar în cazuri
    particulare, grad C).
- Cod intern `RI - PC6` scos de pe ambele rânduri (nu mai aparțin RI — precedent: ATM, §1 editorial).
- `tools/validate.py`: invariantul „Tip `I`/`A` doar în RI" relaxat să accepte și **Sân**.
  `CLAUDE.md` actualizat (regula de aur 4, ierarhia regula 2, invariantul, „Decizii deja luate",
  maparea). NR.CRT păstrate (necontigue în Sân — tolerat). RI: 176→174 rânduri. `GHID.html` regenerat.
...
diagnostic cu contrast, „Neindicat", înlocuită de IRM) a fost adusă la **Cap › ORL**, în
situația „Disfuncții ale articulației temporo-mandibulare", **la finalul** listei de examene
(după Radiografia ATM). Astfel toate opțiunile ATM stau într-un singur loc.

- **RECLASIFICAT Tip `I` → `X`.** Artrografia = radiologie cu contrast (injectare intra-
  articulară + imagistică), deci diagnostic, nu intervențional/terapeutic (`Terapeutic` era
  deja `Nu`). Reclasificarea era **necesară**: invariantul „0 rânduri Tip I/A în afara RI".
- **DE-DUPLICAT.** Existau 3 rânduri de artrografie ATM în RI: unul misîncadrat la
  RI › Aparat digestiv (dublură exactă) și o pereche la RI › Sistem nervos cu **grade
  discordante** (`?` vs `B`, semnalată în §1). Păstrat rândul mai complet (grad **B**,
  cu cod), eliminate celelalte două. (−2 rânduri în total.)
- **ELIMINAT codul `RI - PC8`** din `Alte informații` — cod din taxonomia RI, contradictoriu
  pe un rând acum diagnostic în ORL. (Grad B ales dintre `?`/`B`: versiunea codată; dacă
  editorii preferă altfel, se ajustează.)
- NR 1386 păstrat (rândul se mută cu ID-ul atașat) → găuri NR.CRT la 1283, 1385, 1386.
...
### ADĂUGAT + MODIFICAT — rezolvarea celor 2 propuneri de conținut rămase din §22.B (2026-07-09)

Din `EDITORIAL-decisions.md` §22.B: 2 goluri de conținut semnalate față de ghidurile
internaționale, ambele aplicate acum la cererea editorului.

- **ADĂUGAT — situație nouă „Abces / infecție profundă de spații cervicale"** (subcapitol
  „Mase cervicale", poziționată prima, alfabetic). 2 rânduri:
  - **NR 1441** T, CT regiune cervicală cu substanță de contrast, Indicat, grad **B** —
    standardul de diagnostic pentru infecțiile profunde ale spațiilor cervicale
    (retrofaringian, parafaringian, angina Ludwig): extensie, localizarea colecției,
    complicații (extensie mediastinală, tromboză jugulară, erodare vasculară); câmpul de
    examinare extins la mediastinul superior dacă se suspectează extensie descendentă
    (spațiile visceral/retrofaringian/prevertebral comunică cu mediastinul).
  - **NR 1442** E, Ecografie cervicală/intraorală, Doar în cazuri particulare, grad **B** —
    pentru abcesul periamigdalian (cea mai frecventă localizare, superficială, accesibilă):
    sensibilitate/specificitate comparabile cu CT (89–100%), fără iradiere, poate ghida
    direct drenajul; rezervată colecțiilor superficiale — pentru localizări profunde sau
    suspiciune de complicații, CT rămâne necesar.
  - Surse: Springer — *Imaging of Neck and Facial Infections*; *Retropharyngeal Abscess:
    Patterns of Deep Neck Space Extension* (2024); JETem — *POCUS for Peritonsillar
    Abscess*. Referințe în „Alte informații" pe ambele rânduri.
- **MODIFICAT (îmbogățire) — „Obstrucția fluxului salivar" (Glande salivare), sialadenita
  acută fără litiază.** Gol mai îngust decât inițial estimat: situația acoperea deja bine
  litiaza (NR 1021 menționează „natura litiazică", NR 1023 menționează complicația
  infecțioasă); lipsea doar cazul **sialadenitei acute infecțioase fără calcul** (bacteriană,
  tipic la pacienți deshidratați/postoperator). Adăugată frază la finalul comentariului
  **NR 1021** (Ecografie): aspectul ecografic (glandă difuz mărită, hipoecogenă, hiperemică
  Doppler) și confirmarea că ecografia rămâne prima intenție și în absența litiazei. Surse:
  StatPearls — *Sialolithiasis*; Applied Radiology — *Obstructive Sialadenitis*. Referințe
  adăugate în „Alte informații" pe NR 1021 și NR 1023.

**Nemodificat**: gradele/dozele/ordinea pe situațiile atinse de îmbogățire. Bilanț:
**+2 rânduri** → 1438 → **1440 rânduri**.
...
- **ADĂUGAT (primit din Aparat locomotor, 2026-07-10) —** situația **„Nevralgie
  cervico-brahială"** (Coloană cervicală) primește un rând **Mielo-CT** (Tip T, „Doar în cazuri
  particulare"/B), relocat din fostul Aparat locomotor › Umăr la reconcilierea situației omonime
  (vezi §13 › MUTAT/COMASAT). Grupul cervical devine CT · Mielo-CT · IRM · Radiografie standard.
  NR.CRT păstrat (1155 — gap-jumper în capitol, tolerat).
...
### MUTAT / COMASAT — încadrări rezolvate (2026-07-10, la cerere; ex-EDITORIAL §32.A–B)

- **MUTAT — „Fasceita plantară" din „Diverse" → subcapitolul „Picior".** Cele 4 rânduri
  (N/E/M/G) relocate fizic lângă „Halux valgus", ordinea examenelor păstrată. „Picior" nu mai
  e subcapitol cvasi-gol; „Diverse" păstrează doar condițiile sistemice/cross-regiune
  (artropatie inflamatorie, osteomalacie, osteomielită, sindrom dureros post-protezare).
  NR.CRT neschimbate. (Aparat locomotor rămâne la 74 → apoi 70 după punctul următor.)
- **MUTAT + COMASAT — „Nevralgie cervico-brahială" scoasă din „Umăr" → Coloană vertebrală ›
  Coloană cervicală** (unde exista **deja** o situație omonimă). Reconciliere de duplicat, nu
  mutare oarbă: cele 3 modalități suprapuse (CT, IRM, Radiografie) aveau **grade identice** cu
  rândurile existente în Coloană cervicală → **comasate** (păstrate rândurile canonice de acolo,
  deja revizuite). Singurul conținut nou — **Mielo-CT** (Tip T, „Doar în cazuri particulare"/B) —
  a fost relocat în grupul cervical (după rândul CT), completând comentariul CT existent care
  menționa deja mieloCT ca opțiune. Typo corectat pe rândul mutat: `țn → în`. **Niciun grad
  schimbat.** Aparat locomotor: 74 → **70 rânduri** (−4 din Umăr, dintre care 1 relocat, 3
...
### MUTAT — Intervenționalul mamar scos din RI › Oncologie → Sân (2026-07-10, la cerere)

Cele 2 rânduri de intervențional mamar (**NR 1178** biopsie ganglionară axilară, **NR 1192**
biopsie percutanată mamară) au fost **mutate din `RI › Oncologie` în capitolul Sân** — excepție
nouă de senologie (vezi detaliul la cap. 9 Sân și `CLAUDE.md`). RI: 176→174 rânduri.

### MUTAT / ELIMINAT — Artrografia ATM scoasă din RI (2026-07-10, la cerere)

Artrografia articulației temporo-mandibulare (3 rânduri Tip `I`, „Neindicat") a fost scoasă
din RI: unul relocat la **Cap › ORL** (reclasificat Tip `X` diagnostic — vezi §10) și **2
eliminate** ca duplicate (unul misîncadrat la RI › Aparat digestiv, unul din perechea cu grade
discordante de la RI › Sistem nervos). Rezolvă `EDITORIAL-decisions.md` §1. (−3 rânduri din RI.)
